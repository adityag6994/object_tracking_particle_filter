...
with open(sys.argv[1],'r') as csvfile:
    ...
    for row in plots:
        ...
        roll.append((float(row[3]))+45)
        pitch.append((float(row[4]))-20)
        yaw.append(float(row[5])+45)
        roll_lpf.append((float(row[9]))+45)
        pitch_lpf.append((float(row[10]))-20)
        yaw_lpf.append(float(row[11])+45)     
        # if(count == 300):
            # break;
...
lx,=ax1.plot(iii,x_lpf, color='r')
ly,=ax2.plot(iii,y_lpf, color='r')
lz,=ax3.plot(iii,z_lpf, color='r')
lxO,=ax1.plot(iii,x, color='g')
lyO,=ax2.plot(iii,y, color='g')
lzO,=ax3.plot(iii,z, color='g')

l1x = ax1.axhline(y=mean_x_lpf,c="k",linewidth=1.5,zorder=0)
l1y = ax2.axhline(y=mean_y_lpf,c="k",linewidth=1.5,zorder=0)
l1z = ax3.axhline(y=mean_z_lpf,c="k",linewidth=1.5,zorder=0)
...
ax1.legend([lx, l1x, lxO],["x After LPF", "Mean", "Orignal"])
ax2.legend([ly, l1y, lyO],["y After LPF", "Mean", "Orignal"])
ax3.legend([lz, l1z, lzO],["z After LPF", "Mean", "Orignal"])

plt.show()

# f, (ax1, ax2, ax3, ax4) = plt.subplots(4, sharex=True, sharey=True)
# l1,=ax2.plot(iii,roll, color='r', label='Blue stars')
# l2,=ax1.plot(iii,pitch, color='r')
# l3,=ax3.plot(iii,yaw, color='r')
# lr,=ax2.plot(k,current_roll, color='g')
# l4,=ax2.plot(iii,roll_lpf, color='b', label='Blue stars')
# l5,=ax1.plot(iii,pitch_lpf, color='b')
# l6,=ax3.plot(iii,yaw_lpf, color='b')
# lx,=ax4.plot(iii,x_lpf, color='r')
# ly,=ax4.plot(iii,y_lpf, color='g')
# lz,=ax4.plot(iii,z_lpf, color='b')
# lrLPF,=ax2.plot(k,current_roll, color='g')
...
# # l11 = ax2.axhline(y=mean_roll,c="g",linewidth=1.5,zorder=0)
# l12 = ax1.axhline(y=mean_pitch,c="g",linewidth=1.5,zorder=0)
# l13 = ax3.axhline(y=mean_yaw,c="y",linewidth=1.5,zorder=0)
# # l14 = ax2.axhline(y=mean_roll_lpf,c="g",linewidth=1.5,zorder=0)
# l15 = ax1.axhline(y=mean_pitch_lpf,c="g",linewidth=1.5,zorder=0)
# l16 = ax3.axhline(y=mean_yaw_lpf,c="y",linewidth=1.5,zorder=0)

# print(len(k), ' ' , len(iii))
# # for q in range(1,100):
# #     print(k[q], ' ', iii[q])
# # plt.legend([l1, l2, l3, l4, l5, l6],["Pitch", "Yaw", "Roll","PitchLPF", "YawLPF", "RollLPF"])
# ax2.legend([l1,lr,l4],["Estimate","Reference","After LPF"])
# ax1.legend([l2,l13,l5],["Estimate","Reference","After LPF"])
# ax3.legend([l3,l16,l6],["Estimate","Reference","After LPF"])
# ax4.legend([lx,ly,lz],["x","y","z"])
# plt.show()
